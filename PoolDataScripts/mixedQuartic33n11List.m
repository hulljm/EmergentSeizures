%mixed quartics for quadratics 3,3,1,1 other=3 self=1
%RRRR
if(polyorder>=2)
    % poly order 2
    for i=1:nVars
...
if(polyorder>=2)
    ...
    for i=1:nVars
       % for j=1:nVars
            ...
            str1 = sprintf('1./(1*abs(y(%d)+1i*y(%d)).^(%d)+1).*1./(1*abs(y(%d)+1i*y(%d)).^(%d)+1).*y(%d).*y(%d).*y(%d).*y(%d)/1976.0045 ',realSelf,imagSelf,normFactor1,realOther,imagOther,normFactor2, imagOther,imagOther,realSelf,imagSelf);
            strOut=append(str1);
            youtBuilder{ind,1}=[strOut];
            ind = ind+1;
    end
