...
%% Libraries 7-14 (Cross regional, no mixed products)
% script that performs an expansion and sparsification of the Library 7-14 addition
% matrixes. 

% This script differs from Library 03to06 in that functions are now possible
% between regions (but limited to not including mixed products). 
...
whichResultsRowAddition=32; %This is the row of the addition with results written relative to this
whichResultsRow=whichResultsRowAddition;
autScore=0;crossFreqScore=0; crossRegScore=1; %specficy library level for scoring
isCrossFrequencyPresent=0; %change to 1 for libraries where cross frequency coupling is present
%
...
%Cull Addition: steps 
    ...
    %cull region its from (Left/Right: Transcallosal)
    whichResultsRow=whichResultsRowAddition+1;
    modelScoreOutputPrime=resultsCellOutput{whichResultsRowAddition,3};
    ...
    XiProductsTemp_allCutsReal_CullAdditionProd=resultsCellOutput{whichResultsRow-1,10}-XiPrimeReal;
    XiProductsTemp_allCutsImag_CullAdditionProd=resultsCellOutput{whichResultsRow-1,11}-XiPrimeImag;
    %
    cullRegItsFromFrontBack; %script that loops through remaining parts of addition from previous step
    %
    XiConstrainedReal=XiProductsTemp_allCutsReal_CullAdditionProd+XiPrimeReal;
    XiConstrainedImag=XiProductsTemp_allCutsImag_CullAdditionProd+XiPrimeImag;
    runBoth
