...
%% Libraries 3,4,5,6 Within Region, Between Band Mixed Products
% script that performs an expansion and sparsification of the Library 3-6 addition
% matrixes. 

% This script differs from Library 2 in that functions are now possible
% between mixed products between bands (but still limited to be within
% region). The increased size also breaks the functions into seperate
% libraries based on product order. Library 3=quadratic and cubic, 4,5,6
% = the different quartic functions. The variable whichResultsRowAddition should be changed to match
% the appropriate row in resultsCellOutput for each library
...
whichResultsRowAddition=17; %This is the row where the results will be written to
whichPreviousRow=whichResultsRowAddition-1;
...
%Define XiPrime, XiAddition, XiPrimeUXiAddtion
    XiPrimeReal=resultsCellOutput{whichPreviousRow,10};
    XiPrimeImag=resultsCellOutput{whichPreviousRow,11};
...
%Cull Addition: steps 
    ...
    includeOrder=[1,1,1];
    %
    cullBandItsFrom_general; %script that loops through Addition
    %
    XiConstrainedReal=XiProductsTemp_allCutsReal_CullAdditionProd_Old+XiPrimeReal;
    XiConstrainedImag=XiProductsTemp_allCutsImag_CullAdditionProd_Old+XiPrimeImag; 
    runBoth
    ...
    %
    cullBandItsIn_general; %script that loops through remaining parts of addition from previous step
    %
